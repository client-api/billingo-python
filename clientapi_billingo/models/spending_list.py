...
class SpendingList(BaseModel):
    """
    An object with a data property that contains an array of up to limit spending. Each entry in the array is a separate spending object. If no more spending are available, the resulting array will be empty.  # noqa: E501
    """
    data: Optional[conlist(SpendingListItem)] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> SpendingList:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> SpendingList:
        """Create an instance of SpendingList from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return SpendingList.parse_obj(obj)

        _obj = SpendingList.parse_obj({
            "data": [SpendingListItem.from_dict(_item) for _item in obj.get("data")] if obj.get("data") is not None else None,
            "total": obj.get("total"),
