...
class PartnerList(BaseModel):
    """
    A object with a data property that contains an array of up to limit partners. Each entry in the array is a separate partner object. If no more partners are available, the resulting array will be empty.  # noqa: E501
    """
    data: Optional[conlist(Partner)] = None
    ...
    @classmethod
    def from_json(cls, json_str: str) -> PartnerList:
    ...
    @classmethod
    def from_dict(cls, obj: dict) -> PartnerList:
        """Create an instance of PartnerList from a dict"""
        if obj is None:
            return None

        if not isinstance(obj, dict):
            return PartnerList.parse_obj(obj)

        _obj = PartnerList.parse_obj({
            "data": [Partner.from_dict(_item) for _item in obj.get("data")] if obj.get("data") is not None else None,
            "total": obj.get("total"),
